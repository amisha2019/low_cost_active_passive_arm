...
class ArmOffset:
    def __init__(self, device_name, baudrate):
        ...
        self.rounded_curr_pos = np.round(rounded_values, 2)

        # Joints in a specific configurations
        self.reset_joints = np.array([-1.57, -1.57, -1.57, -1.57, 1.57, 1.57])

        # Correction required
        self.correction_joints = self.rounded_curr_pos - self.reset_joints
...
offset = ArmOffset(device_name="/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT8J0W3F-if00-port0", baudrate=57600)
# Always subtract the correction joints from the current joints
print("Offset: ", offset.rounded_curr_pos)
print("Corrections: ", offset.correction_joints)  
print("After Correction: ", offset.rounded_curr_pos - offset.correction_joints)